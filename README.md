...
(Replace "Â­binÂ­" by "Â­ScrÂ­iptÂ­s" in Windows).
```
source env_naÂ­meÂ­\binÂ­\acÂ­tivate
```
...
```
- Create requirÂ­ements file.
```
pip freeze > requirÂ­emeÂ­nts.txt
```
- Install all required dependÂ­encies based on your pip freeze command.
```
pip install -r requirÂ­emeÂ­nts.txt
```
...
- Creating an `my_app` directory and all default files/fÂ­olders inside.
```
...
```
urlpatÂ­terns = [
 path('admin/', admin.sÂ­itÂ­e.uÂ­rls),
...
```
All the operation on the student database table have to be done using the â€œauthorsâ€ manager
```
...
> If you use custom Manager objects, take note that the first Manager Django encounters (in the order in which theyâ€™re defined in the model) has a special status. Django interprets the first Manager defined in a class as the â€œdefaultâ€ Manager, and several parts of Django (including dumpdata) will use that Manager exclusively for that model. As a result, itâ€™s a good idea to be careful in your choice of default manager in order to avoid a situation where overriding get_queryset() results in an inability to retrieve objects youâ€™d like to work with.
...
Uses SQLâ€™s `UNION` operator to combine the results of two or more QuerySets.
For example:
```
>>> qs1.union(qs2, qs3)
```
...
Passing different models works **as long as the SELECT list is the same in all QuerySets** (at least the types, the names donâ€™t matter as long as the types are in the same order).
...
The route argument should be a string or `gettext_lazy()` that contains a regular expression compatible with Pythonâ€™s re module.
...
- **include()**: A function that takes a full Python import path to another URLconf module that should be â€œincludedâ€ in this place.
...
- Set the exclude attribute of the ModelFormâ€™s inner Meta class to a list of fields to be excluded from the form.
...
select_related returns a QuerySet that will â€œfollowâ€ foreign-key relationships (either One-to-One
...
`prefetch_related` performs a separate lookup for each relationship and â€œjoinsâ€ them together
with Python, not SQL.  
...
This works best if youâ€™ve got a fast, well-indexed database server.
In this example, the cache tableâ€™s name is `my_cache_table`:
